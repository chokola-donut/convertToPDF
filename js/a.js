        ...
        Array.from(paragraph.childNodes).forEach((node) => {
            ...
            if (node.nodeType === Node.TEXT_NODE) {
                ...
                textLines.forEach((line) => {
                    if(line.className == 'title'){
                        line.textColor(179, 179, 179)
                    }
...
function addBoxesHelper(boxes,textStyle,xOffset,yOffset1,yOffset1){
    const textLines = doc.splitTextToSize(boxes[0].innerText, textStyle.maxWidth);
            textLines.forEach((line) => {
                ...
                const pageHeight = doc.internal.pageSize.height; 
                if (yOffset > pageHeight - 30 && yOffset - yOffset1 > 20){ 
                    doc.line(10, yOffset, 10, yOffset1-5); 
                    doc.addPage();
                    yOffset = 20;
                    yOffset1 = 20;
                }
...
function addBoxes(boxes) {
    ...
    if(boxes[0].classList.contains('admonition-quote')){
            ...
            addBoxesHelper(boxes,textStyle,xOffset,yOffset1,yOffset1)
...
function addImgs(img){
    ...
    Array.from(img.childNodes).forEach((node) => {
        checkPageBreak();
            let xOffset=20
    ...
    if (node.tagName === 'EM') {
        const italicText = doc.splitTextToSize(node.textContent, textstyle.maxWidth - xOffset);
        ...
        italicText.forEach((line) => {
            ...
            doc.text(line, doc.internal.pageSize.width/2, yOffset,{align: textstyle.halign });
            xOffset += doc.getTextWidth(line);
        });
    ...
    function addfile(links) {
        links.forEach((link) => {
            const url = link.getAttribute('href'); // Используем 'url' для объекта ссылки
            const text = link.textContent;
            
            // Добавляем текст ссылки в PDF
            doc.setTextColor(0, 0, 238)
            doc.text(text, 10, yOffset);
            console.log(text.length)
            console.log(text)
            // Создаем ссылку в PDF
            doc.link(10, yOffset - 5,text.length*2, 10, { url });
            
            console.log(text, "____", url);
            yOffset += 15; // Перемещаем yOffset для следующей строки
        });
        doc.setTextColor(0, 0, 0)  
    }
...
function a(el){
    if (el.classList.contains('title1')) {
        addTitles([el]);
    } else if (el.classList.contains('paragraph')||el.tagName==="P") {
        addParagraphs([el]);
    ...
    else if (el.tagName === "DIV" && el.firstElementChild && el.firstElementChild.tagName === "IMG") { 
  console.log(el)
         addImgs(el)
 }
    ...
    else if(el.tagName==="GR-FILE"){
     addfile([el])
}
    if (!(el.tagName === "DIV" && el.firstElementChild && el.firstElementChild.tagName === "IMG")) {
